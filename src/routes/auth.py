...
async def signup(
    ...
    An email for email address confirmation is sent to the newly created users' email.

    :param body: The data for the new user to create.
    :type body: UserModel
    :param background_tasks: Background task to run.
...
async def login(
    ...
    """
    User authentication.
    Gets the credentials provided in the body, checks for availability, and matches against the database.
    On success, returns an access token and an update.
...
async def refresh_token(
    ...
    :param credentials: HTTP authorization credentials containing a refresh token.
